...
void MLS::addToOccupancy(pcl::PointCloud<pcl::PointXYZ>::Ptr input_cloud) {
  ...
  for (int i = 0; i < drv_cloud->size(); i++) {
    ...
    // check range
    if (x < 0 || y < 0 || x >= size_x || y >= size_x || rng > max_range)
      continue;
...
void MLS::updateCell(int x, int y) {
  ...
  // recalculate mean/cov for drivability
  for (int j = 0; j < cell->cloud.size(); j++) {
    ...
    for (int c = 0; c < cell->clusters.size(); c++) {
      ...
      if (cell->clusters[c].num_pts < min_cluster_points) {
        ...
        if (cur_dist <
            (sqrt(cell->clusters[c].cov(2, 2)) * cluster_sigma_factor +
    ...
    cluster->mean[0] =
        (((cluster->num_pts) - 1) / (cluster->num_pts)) * (cluster->mean[0]) +
        1 / (cluster->num_pts) *
            pt.x; // FIXME x/y mean be broke for moving cells
    cluster->mean[1] =
        (((cluster->num_pts) - 1) / (cluster->num_pts)) * (cluster->mean[1]) +
        1 / (cluster->num_pts) * pt.y;
    cluster->mean[2] =
        (((cluster->num_pts) - 1) / (cluster->num_pts)) * (cluster->mean[2]) +
    ...
    if (cluster->num_pts > 1) {
      cluster->cov(2, 2) =
          ((cluster->num_pts - 1) / (cluster->num_pts)) * (cluster->cov(2, 2)) +
          1.0 / (cluster->num_pts - 1) * (pt.z - cluster->mean[2]) *
              (pt.z - cluster->mean[2]);
      cluster->cov(2, 2) =
  ...
  if (ground_idx + 1 < cell->clusters.size() &&
      cell->clusters[ground_idx + 1].num_pts > min_cluster_points) {
    double clearance = cell->clusters[ground_idx + 1].mean[2] -
                       sqrt(cell->clusters[ground_idx + 1].cov(2, 2)) * 2 -
                       cell->clusters[ground_idx].mean[2]; // 2 sigma clearance
    if (clearance < cluster_combine_dist) {

      // combine clusters
      double ratio0 = cell->clusters[ground_idx].num_pts /
                      (cell->clusters[ground_idx].num_pts +
                       cell->clusters[ground_idx + 1].num_pts);
      double ratio1 = cell->clusters[ground_idx + 1].num_pts /
                      (cell->clusters[ground_idx].num_pts +
                       cell->clusters[ground_idx + 1].num_pts);
      cell->clusters[ground_idx].mean[0] =
          ratio0 * (cell->clusters[ground_idx].mean[0]) +
          ratio1 * (cell->clusters[ground_idx + 1].mean[0]);
      cell->clusters[ground_idx].mean[1] =
          ratio0 * (cell->clusters[ground_idx].mean[1]) +
          ratio1 * (cell->clusters[ground_idx + 1].mean[1]);
      cell->clusters[ground_idx].mean[2] =
          ratio0 * (cell->clusters[ground_idx].mean[2]) +
          ratio1 * (cell->clusters[ground_idx + 1].mean[2]);
      cell->clusters[ground_idx].cov(2, 2) =
          (ratio0 * cell->clusters[ground_idx].cov(2, 2) +
           ratio1 * cell->clusters[ground_idx + 1].cov(2, 2)); // FIXME??
      cell->clusters.erase(cell->clusters.begin() + ground_idx + 1);
    } else if (clearance < robot_height) {
      cell->drivable = 0;
      drivabilityGrid->data[x + size_x * y] = 100;
      return;
    }
  ...
  for (int i = -1; i <= 1; i++) {
    for (int j = -1; j <= 1; j++) {
      if ((i == 0 && j == 0) || i + x < 0 || i + x >= size_x || j + y < 0 ||
          j + y >= size_y)
        continue;
      if (grid(x + i, y + j)->updated == true) {
        updateCell(x + i, y + j); // OH GOD IT'S RECURSIVE!!!
      }
      if (!grid(x + i, y + j)->clusters.empty() &&
          grid(x + i, y + j)->clusters[0].num_pts > min_cluster_points) {
        double ndiff =
            cell->clusters[0].mean[2] - grid(x + i, y + j)->clusters[0].mean[2];
  ...
  // check covariance
  if (abs(cell->clusters[ground_idx].cov(2, 2)) > thickness_threshold) {
    cell->drivable = 0;
    drivabilityGrid->data[x + size_x * y] = 100;
    return;
  }
...
void MLS::addToMap(pcl::PointCloud<pcl::PointXYZI>::Ptr input_cloud) {
  ...
  // (0,0)
  // For global map we assume that the cloud has been translated into the global
  // frame
  ...
  if (!disable_pointcloud) {
    // we should probably filter this
    *global_cloud += *input_cloud; // this could get very large!! make usre to
                                   // use filterCloud method
  }
}
    ...
    for (int i = 0; i < size_x; i++) {
      for (int j = 0; j < size_y; j++) {
        ...
        if (grid(i, j)->drivable < 0) {
          ...
          drivabilityGrid->data[i + size_x * j] =
              (grid(i, j)->drivable == 0) ? 100 : 0;
        }
      }
    }
  }
...
void MLS::getSegmentedClouds(pcl::PointCloud<pcl::PointXYZ>::Ptr &obstacle,
...
void MLS::updateElevationGrid() {
  elevationGrid.clear("elevation");
  ...
  for (unsigned int i = 0; i < size_x; i++) {
    for (unsigned int j = 0; j < size_y; j++) {
      ...
      double min_cell_height = 1000;

      for (unsigned int c = 0; c < cell->clusters.size(); c++) {
        if (cell->clusters[c].mean[2] < min_cell_height) {
          min_cell_height = cell->clusters[c].mean[2];
        }
      }

      if (cell->clusters.size() < 1)
        min_cell_height = std::numeric_limits<double>::quiet_NaN();
      // update elevation cell
      elevationGrid.at("elevation", grid_map::Index(i, j)) = min_cell_height;
    }
  }
}

grid_map::GridMap MLS::getElevationGrid() {
  updateElevationGrid();
  ...
  for (int i = -update_dist + curX; i < update_dist + curX; i++) {
    for (int j = -update_dist + curY; j < update_dist + curY; j++) {
      ...
      for (int c = 0; c < cell->clusters.size(); c++) {
        ...
        // diagMat(0,0) = sqrt(lx);diagMat(1,1) = sqrt(ly);diagMat(2,2) =
        // sqrt(lz);
        //
        ...
        marker.scale.x =
            resolution; // FIXME use proper value here  //sqrt(lx)*3;
