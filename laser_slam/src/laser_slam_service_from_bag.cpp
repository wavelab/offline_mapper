...
int main(int argc, char **argv)
{
    ...
    //ROS PUBLISHERS
    ros::Publisher map_cloud_pub = nh.advertise<sensor_msgs::PointCloud2>("/laserslam/full_pointcloud", 1, true);
    ros::Publisher drivablility_pub = nh.advertise<nav_msgs::OccupancyGrid>("/laserslam/drivability_map", 1, true);
	ros::Publisher elevation_pub = nh.advertise<grid_map_msgs::GridMap>("/laserslam/elevation_map", 1, true);
	ros::ServiceClient client = nh.serviceClient<laser_slam::InsertPointCloud>("/laser_slam/InsertPointCloud");
	...
	rosbag::View view(bag, rosbag::TopicQuery(topics), ros::TIME_MIN, ros::TIME_MAX, true);
	...
	BOOST_FOREACH(rosbag::MessageInstance const m, view)
    {
        ...
        if(m.getTopic() == ekf_topic)
        {
        	...
        	anm_msgs::VehicleState::ConstPtr vehicle_msg = m.instantiate<anm_msgs::VehicleState>();
        	...
        	if (vehicle_msg != NULL)
        	{
        		...
        		pose_msg.orientation = tf::createQuaternionMsgFromRollPitchYaw(vehicle_msg->orientation.x, vehicle_msg->orientation.y, vehicle_msg->orientation.z);
        	...
        	{
        		ROS_ERROR("position message returned null pointer. Check your message definitions!");
        	}
