...
double ScanRegistration::getNearestKF(PoseGraph &pG, Node &gN) {
  ...
  for (int i = 0; i < numKF; i++) {
    ...
    dx = currNode.pose.pose.position.x - gN.pose.pose.position.x;
    dy = currNode.pose.pose.position.y - gN.pose.pose.position.y;
...
std::vector<int> ScanRegistration::getKNN(PoseGraph &pG, Node &gN,
                                          unsigned int K) {
...
Eigen::Matrix6d ScanRegistration::computeEdgeInformationLUM(
    pcl::PointCloud<pcl::PointXYZ>::Ptr &source_trans,
    pcl::PointCloud<pcl::PointXYZ>::Ptr &target, double max_corr_distance) {
  ...
  for (int i = 0; i < numSourcePts; i++) {
    ...
    kdtree.nearestKSearch(
        qpt, 1, nn_idx,
        nn_sqr_dist); // returns the index of the nn point in the target

    if (nn_sqr_dist[0] <
        max_corr_distance * max_corr_distance) // if the distance to point is
                                               // less than max correspondence
                                               // distance, use it to calculate
    {
...
bool ScanRegistration::calcEdgeIcp(Edge &gE, PoseGraph &pG, double scale_div) {
  ...
  registration_icp.setMaxCorrespondenceDistance(course_scale_icp_distance_max /
                                                scale_div);
  ...
  pcl::PointCloud<pcl::PointXYZ>::Ptr from_cld(
      new pcl::PointCloud<pcl::PointXYZ>);
  pcl::PointCloud<pcl::PointXYZ>::Ptr to_cld(
      new pcl::PointCloud<pcl::PointXYZ>);
  ...
  edge_information = computeEdgeInformationLUM(
      temp, from_cld,
      course_scale_icp_distance_max /
          scale_div); // the max here should be the actual icp dmax
  ...
  tf::Matrix3x3 tf3d;
  tf3d.setValue(static_cast<double>(transformation(0, 0)),
                static_cast<double>(transformation(0, 1)),
                static_cast<double>(transformation(0, 2)),
                static_cast<double>(transformation(1, 0)),
                static_cast<double>(transformation(1, 1)),
                static_cast<double>(transformation(1, 2)),
                static_cast<double>(transformation(2, 0)),
                static_cast<double>(transformation(2, 1)),
  ...
  double theta_diff = fabs(tf::getYaw(initPose.pose.orientation) -
                           tf::getYaw(edge.pose.orientation));
  ...
  if ((x_diff > icp_trans_threshold) || (y_diff > icp_trans_threshold) ||
      (theta_diff > icp_rot_threshold)) // reject
  {
...
bool ScanRegistration::addEdgeToX(PoseGraph &pG, int from, int to,
                                  double scale_div) {
