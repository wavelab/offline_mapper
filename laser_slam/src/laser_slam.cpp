...
{	
    ...
    for(unsigned int i=0; i<pose_graph->nodes.size(); i++)
    {
        ...
        pcl::transformPointCloud(*(pose_graph->nodes[i].keyframe),*trans_cloud, trans);
        global_map->addToMap(trans_cloud, pose_graph->nodes[i].pose);
    } 
  
}

bool LaserSlam::insertAndProcess(pcl::PointCloud<pcl::PointXYZ>::Ptr input_cloud, Eigen::Affine3d input_pose){
    ...
    pcl::PointCloud<pcl::PointXYZ>::Ptr gseg_output (new pcl::PointCloud<pcl::PointXYZ>);
    ...
    *gN.keyframe = *gseg_output;
    ...
    GSO->addPosePrior(gN);
     ...
     if(pose_graph->nodes.size()<2) //can't do anything unless we have more than 1 node
     {
        regenerateGlobalMap();
        return true;
     }
    ...
    for(int ref_iter =0; ref_iter< this->num_refinement_iter; ref_iter++)
    {
        ...
        {	
	        ...
	        for(unsigned int j=0; j<pose_graph->nodes.size(); j++)
	        {
	            ...
	            vector<int> KFNN = scan_registration->getKNN(*pose_graph, current_node, this->num_vertex_knn);
	            ...
	            for(unsigned int i = 0; i<KFNN.size(); i++) {
	                ...
	                if(scan_registration->addEdgeToX(*pose_graph, KFNN[i], current_node.idx,scale_div)) {
	                    ROS_INFO_STREAM("Laser SLAM::Adding edge between scans " << current_node.idx << " and " << (KFNN[i]) << " with scale division " << scale_div);
    ...
    ROS_INFO_STREAM("Laser SLAM::Total Insertion Time: " << (end_time-start_time) << "s");
    return true;
}


bool LaserSlam::insertPointCloud(laser_slam::InsertPointCloud::Request  &req, laser_slam::InsertPointCloud::Response &res)
{
  ...
  ROS_INFO("Laser SLAM::Finished Pointcloud insertion, Waiting for next point cloud");
