...
bool GraphSlamOptimizerGTSAM::addVertex(Node &node) {
  ...
  Key id = node.idx;
  Rot3 R = Rot3::quaternion(
      node.pose.pose.orientation.w, node.pose.pose.orientation.x,
      node.pose.pose.orientation.y, node.pose.pose.orientation.z);
  Point3 t = Point3(node.pose.pose.position.x, node.pose.pose.position.y,
  ...
  if (node.idx == 0) // first one, add a prior factor
  {
    ...
    noiseModel::Diagonal::shared_ptr priorModel =
        noiseModel::Diagonal::Variances(v6);
    Key firstKey = 0;
    graph.add(PriorFactor<Pose3>(firstKey, Pose3(R, t), priorModel));
  }
  return true;

  /*const double pose[7] = {node.pose.pose.position.x,
  node.pose.pose.position.y, node.pose.pose.position.z,
  node.pose.pose.orientation.x, node.pose.pose.orientation.y,
  node.pose.pose.orientation.z, node.pose.pose.orientation.w};
...
bool GraphSlamOptimizerGTSAM::addEdge(Edge &gE) {
  ...
  Key id2 = gE.to;
  Rot3 R =
      Rot3::quaternion(gE.edge.pose.orientation.w, gE.edge.pose.orientation.x,
                       gE.edge.pose.orientation.y, gE.edge.pose.orientation.z);
  Point3 t = Point3(gE.edge.pose.position.x, gE.edge.pose.position.y,
                    gE.edge.pose.position.z);
  ...
  /*const double transf[7] = {gE.edge.pose.position.x,gE.edge.pose.position.y,
  gE.edge.pose.position.z,
  gE.edge.pose.orientation.x,gE.edge.pose.orientation.y,gE.edge.pose.orientation.z,
...
void GraphSlamOptimizerGTSAM::optimizeGraph() {
  ...
  params.setVerbosity(
      "TERMINATION"); // this will show info about stopping conditions
