...
template <typename TYPE> void LoadParameter(std::string param_name, TYPE& param_var, ros::NodeHandle& nh)
{
...
int main(int argc, char **argv)
{
    ...
    LoadParameter<int>("scanreg_num_refinement_iterations",scanreg_num_ref_iter,nh);
    LoadParameter<double>("scanreg_init_leaf_size",scanreg_init_leaf_size,nh);
    LoadParameter<double>("scanreg_init_icp_distance_max",scanreg_init_icp_distance_max,nh);
    LoadParameter<double>("scanreg_icp_trans_thresh",scanreg_icp_trans_thresh,nh);
    ...
    LoadParameter<double>("mls_cluster_sigma_factor",mls_cluster_sigma_factor,nh);
    LoadParameter<double>("mls_cluster_dist_threshold",mls_cluster_dist_threshold,nh);
    LoadParameter<double>("mls_cluster_combine_dist",mls_cluster_combine_dist,nh);
    LoadParameter<bool>("scanreg_recompute_all_edges",scanreg_recompute_all_edges,nh);
    ...
    LaserSlam laser_slam(map_x_cells, map_y_cells, map_resolution, mls_vehicle_height,scanreg_init_leaf_size,scanreg_init_icp_distance_max,scanreg_icp_trans_thresh,scanreg_icp_rot_thresh);
    ...
    laser_slam.posegraph_pub = nh.advertise<visualization_msgs::MarkerArray>("/laserslam/pose_graph", 1, true);
    laser_slam.mls_pub = nh.advertise<visualization_msgs::MarkerArray>("/laserslam/mls", 1, true);
    laser_slam.service = nh.advertiseService("InsertPointCloud", &LaserSlam::insertPointCloud, &laser_slam);
