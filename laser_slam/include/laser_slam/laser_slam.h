...
public:
  ...
  LaserSlam(int x_size, int y_size, double map_res, double vehicle_h,
    ...
    scan_registration = new ScanRegistration(init_leaf_size, icp_distance_max,
  ...
  // public functions
  bool insertPointCloud(laser_slam::InsertPointCloud::Request &req,
                        laser_slam::InsertPointCloud::Response &res);
  int run();

  // accessors
  void setNumRefinementIterations(int ref_iter) {
    num_refinement_iter = ref_iter;
  }
...
private:
  ...
  bool insertAndProcess(pcl::PointCloud<pcl::PointXYZ>::Ptr input_cloud,
