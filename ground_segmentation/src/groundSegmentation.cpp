...
MatrixXd groundSegmentation::genGPModel(vector<signalPoint> &ps1,
                                        vector<signalPoint> &ps2, float sig_f,
                                        float p_l) {
  int nP1 = ps1.size();
  int nP2 = ps2.size();
...
void groundSegmentation::sectorINSAC(int sectorIndex) {
  ...
  for (int i = 0; i < NUMBINSL; i++) {
    ...
    if (pBG->aCell[sectorIndex].rangeHeightSignal[i].x != INVALID &&
        pBG->aCell[sectorIndex].lCell[i].binPoints.size() >
            5) // bin has a valid point, and enough points to make a good guess
               // for a protopoint
    {
  ...
  while (true) {
    ...
    if (sigPtr[currIdx].range < MAXSEEDRANGE &&
        fabs(sigPtr[currIdx].height) < MAXSEEDHEIGHT) // close enough to robot
                                                      // and height makese sense
                                                      // in robot locality
    {
  ...
  if (sigPtr.size() == 0) // no points to insac, put the seed points in as
                          // ground
    keepGoing = false;
  ...
  while (keepGoing) {
    ...
    // temporary calc
    MatrixXd tCalc1 =
        C_XX + (P_SN * MatrixXd::Identity(C_XX.rows(), C_XX.cols()));
    ...
    if (Vf_s.rows() == 0) {
      keepGoing = false;
      ROS_WARN_STREAM("WARNING BREAKING LOOP: VF_s does not exist"); // arun:
                                                                     // are we
                                                                     // still
                                                                     // getting
                                                                     // this?
    ...
    if (startSize == endSize || sigPtr.size() == 0) // we added no new points in
  ...
  for (int i = 0; i < (int)currentModel.size(); i++) {
    ...
    for (unsigned int j = 0; j < curCell->binPoints.size(); j++) {
      ...
      if (h < P_TG) // z heights are close
      {
        ...
        obsSum += Vector3d(curCell->binPoints[j].x, curCell->binPoints[j].y,
  ...
  int i;
  // cout << "[Local Mapping] WARNING: f_s.size < sigPtr.size, wtf?" << " fsize:
  // " << f_s.size() << " sPtr: " << sigPtr.size() << " did INSAC "<<
  ...
  fsize: " << f_s.size() << " sPtr: " << sigPtr.size() << endl;
  }*/
}
