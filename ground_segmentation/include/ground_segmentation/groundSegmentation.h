 ...
 *  
 *************************************************************************/
#ifndef NDTDATASTRUCTS_H
#define NDTDATASTRUCTS_H
...
#define NUMBINSA 72  //number of angular sectors
#define NUMBINSL 200  //number of linear bins
...
class groundSegmentation {
	...
	//for GP model
	double P_L;          // length parameter, so how close the points have to be in the GP model to consider them correlated
	double P_SF;     // scaling on the whole covariance function
	double P_SN;       // the expected noise for the mode
	double P_TMODEL;  // the required confidence required in order to consider something ground
	double P_TDATA;     // scaled value that is required for a query point to be considered ground
	...
	//constructor
	groundSegmentation();
	void setupGroundSegmentation(pcl::PointCloud<pcl::PointXYZ>::Ptr , pcl::PointCloud<PointXYZGD>::Ptr , pcl::PointCloud<PointXYZGD>::Ptr, pcl::PointCloud<PointXYZGD>::Ptr);
    void genPolarBinGrid(pcl::PointCloud<pcl::PointXYZ>::Ptr );
	void initializePolarBinGrid(void);
    Eigen::MatrixXd genGPModel(vector<signalPoint>& , vector<signalPoint>& ,float ,float );
	void segmentGround(void);	
	void sectorINSAC(int);
		
	//setters
	void set_rmax(double a){ RMAX = a;}
	void set_num_maxbinpoints(int a){ MAXBINPOINTS = a;}
	void set_num_seedpoints(int a){ NUMSEEDPOINTS = a;}
	
	void set_gp_lengthparameter(double a){ P_L = a;}
	void set_gp_covariancescale(double a){ P_SF = a;}
	void set_gp_modelnoise(double a){ P_SN = a;}
	void set_gp_groundmodelconfidence(double a){ P_TMODEL = a;}
	void set_gp_grounddataconfidence(double a){ P_TDATA = a;}
	void set_gp_groundthreshold(double a){ P_TG = a;}
	
	void set_robotheight(double a){ ROBOT_HEIGHT = a;}
	
	void set_seeding_maxrange(double a){ MAXSEEDRANGE = a;}
	void set_seeding_maxheight(double a){ MAXSEEDHEIGHT = a;}
